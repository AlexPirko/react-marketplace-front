...
const favoritesReducer = createReducer(initialState, (builder) => {
    ...
    builder.addCase(removeFromFavoritesAction, (state: any, action: PayloadAction<number>) =>
        state.filter((favoriteId: number) => favoriteId !== action.payload),
    );
});

export const addToFavorites = (favoriteId: number): AnyAction => addToFavoritesAction(favoriteId);

export const removeFromFavorites = (favoriteId: number): AnyAction => removeFromFavoritesAction(favoriteId);
